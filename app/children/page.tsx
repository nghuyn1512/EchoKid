...
export default function ChildrenPage() {
  ...
  async function createChild() {
    ...
    if (response.ok) { setChildren((items) => [...items, data.child]); setForm({ name: "", ageMonths: 24, gender: "male" }); }
        ...
        <section className="profile-picker">
          <div className="profile-grid">
            {loading ? <p>Đang tải hồ sơ...</p> : children.map((child, index) => (
              <Link key={child.id} href={`/analysis?childId=${child.id}`} className="profile-card">
                ...
                <div><h2>{child.name}</h2><p>{child.ageMonths} tháng · {child.gender === "male" ? "Bé trai" : child.gender === "female" ? "Bé gái" : "Khác"}</p></div>
          ...
          <div className="create-profile">
            ...
            <div className="form-grid">
              ...
              <label>Tuổi (tháng)<input type="number" min={6} value={form.ageMonths} onChange={(e) => setForm((v) => ({ ...v, ageMonths: Number(e.target.value) }))} /></label>
